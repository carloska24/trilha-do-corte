    ...
    <div className="w-full max-w-md mx-auto pb-32 px-5 pt-8 animate-fade-in-up">
      ...
      <div className="space-y-5">
        ...
        <SectionCard
          ...
          title="Fidelidade"
          subtitle={loyaltyConfig.enabled ? `${loyaltyConfig.cutsRequired} Cuts = 1 Free` : 'Sistema Offline'}
          sectionId="loyalty"
          expandedSection={expandedSection}
          setExpandedSection={setExpandedSection}
        >
          <div className="space-y-6 pt-2">
            ...
            <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5">
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-full ${loyaltyConfig.enabled ? 'bg-purple-500/20' : 'bg-zinc-800/50'}`}>
                   <Star size={18} className={loyaltyConfig.enabled ? 'text-purple-400' : 'text-zinc-600'} />
                </div>
                <div>
                    <span className="text-white font-bold text-sm block">Status do Sistema</span>
                    <span className="text-[10px] text-zinc-500 uppercase tracking-wider">{loyaltyConfig.enabled ? 'Online' : 'Offline'}</span>
                </div>
              </div>
              <CyberToggle
            ...
            {loyaltyConfig.enabled && (
              ...
              >
                <div className="flex items-center justify-between px-1">
                  <span className="text-zinc-400 text-xs font-bold uppercase tracking-wide">Meta de Cortes</span>
                  <span className="text-purple-400 text-xs font-bold">{loyaltyConfig.cutsRequired} cortes</span>
                </div>
               ...
               <div className="relative overflow-hidden p-0.5 rounded-xl bg-gradient-to-r from-purple-500/50 via-pink-500/50 to-purple-500/50 animate-borderGlow">
                 <div className="bg-black/90 p-4 rounded-[10px] relative">
                    ...
                    <p className="relative text-center text-zinc-300 text-xs font-medium leading-relaxed">
                        ...
                        após completar 
                        <strong className="text-white text-lg mx-1">{loyaltyConfig.cutsRequired}</strong> 
                        cortes.
                    </p>
                 </div>
               </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-white/5 to-transparent rounded-2xl border border-white/5">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-white font-bold text-lg truncate leading-tight">{barberProfile?.name || 'Barber'}</h4>
                <p className="text-zinc-500 text-xs truncate font-mono">{barberProfile?.email}</p>
                <div className="flex items-center gap-2 mt-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 uppercase">Admin</span>
                </div>
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-cyan-950/30 group-hover:bg-cyan-500/20 transition-colors">
                    <Download size={18} className="text-cyan-700 group-hover:text-cyan-400 transition-colors" />
                </div>
                <div className="text-left">
                    <span className="block text-zinc-300 group-hover:text-white font-bold text-sm transition-colors">Download Backup</span>
          ...
          <div className="space-y-6 pt-2">
             ...
             <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-3">
                ...
                <p className="text-[10px] text-red-300/80 leading-relaxed">
                    Área sensível. Ações aqui podem resultar em perda de dados se não executadas com cautela.
                </p>
             </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <Download size={24} className="text-zinc-500 group-hover:text-cyan-400 transition-colors" />
                <span className="text-[10px] font-bold text-zinc-400 group-hover:text-white uppercase tracking-wider">Baixar DB</span>
              </button>
              ...
              <div className="relative h-full">
                <input
                  ...
                  onChange={async e => {
                    ...
                    if (!file) return;
                    if (!confirm('DANGER ZONE: Isso irá substituir TODOS os dados. Deseja continuar?')) {
                      e.target.value = '';
                      return;
                    }
                ...
                >
                  <Save size={24} className="text-zinc-500 group-hover:text-purple-400 transition-colors" />
                  <span className="text-[10px] font-bold text-zinc-400 group-hover:text-white uppercase tracking-wider">Upload DB</span>
                </label>
              </div>
            </div>
              ...
              <div className="p-4 bg-red-950/10 rounded-xl border border-red-500/10 hover:border-red-500/30 transition-colors">
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex-1">
                    <label className="text-[10px] text-zinc-500 uppercase font-bold mb-1.5 block">Limpar antes de:</label>
                    <DatePicker
                      value={cleanupDate}
                ...
                <button
                    onClick={() => {
                        ...
                        ? `ATENÇÃO: Apagar agendamentos ANTERIORES a ${cleanupDate.split('-').reverse().join('/')}?`
                        : 'ATENÇÃO: Apagar agendamentos anteriores a HOJE?';

                        if (!confirm(message)) return;

                        api.post('/maintenance/clean-schedule', { mode: 'past', date: cleanupDate || undefined })
                        .then(res => showToast(`REMOVIDOS: ${res.data.count} ITENS`))
...
const SectionCard = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Glow behind icon */}
        <div className={`absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 blur-xl opacity-20 group-hover:opacity-40 transition-opacity rounded-full ${color.replace('text-', 'bg-')}`} />
        ...
        <div className="flex-1 text-left z-10">
          ...
          {subtitle && <p className="text-zinc-500 text-[10px] font-medium mt-0.5 tracking-wide">{subtitle}</p>}
        </div>
...
const CyberInput = ({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder: string }) => (
    <div>
...
const CyberToggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  ...
  >
    <div className={`absolute top-1/2 -translate-y-1/2 left-1 w-5 h-5 rounded-full shadow-md transition-all duration-300 ${
        checked ? 'bg-purple-400 translate-x-5 shadow-[0_0_10px_rgba(168,85,247,0.5)]' : 'bg-zinc-500 translate-x-0'
    }`} />
  </button>
);
